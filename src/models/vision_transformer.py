...
def get_3d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, temperature=10000.):
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO why this tranformation thing? not in vjepa
        B, L, H, W, D = x.shape
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        pos_embed = self.interpolate_pos_encoding_3d(x, self.pos_embed)
        x = x + pos_embed
    ...
    def interpolate_pos_encoding_3d(self, x, pos_embed):
        npatch = x.shape[1] - 1  # Number of patches in the input (excluding class token)
        N = pos_embed.shape[1] - 1  # Number of patches in the original positional embedding (excluding class token)
        if npatch == N:
            return pos_embed
        # Extract the class embedding if it exists
        class_emb = pos_embed[:, 0]  # Class token embedding
        pos_embed = pos_embed[:, 1:]  # Positional embeddings without the class token        
        # Compute the original 3D grid size from N
        D = H = W = int(round(N ** (1/3)))  # Assumes the original grid is cubic       
        # Reshape pos_embed to 5D tensor (B, C, D, H, W)
        dim = x.shape[-1]
        pos_embed = pos_embed.reshape(1, D, H, W, dim).permute(0, 4, 1, 2, 3)  # (1, dim, D, H, W)       
        # Compute the scaling factors for the 3D grid
        npatch_3d = int(round(npatch ** (1/3)))  # New grid size (cubic assumption)
        scale_factors = (npatch_3d / D, npatch_3d / H, npatch_3d / W)        
        ...
        # Reshape pos_embed back to the original form (B, num_patches, dim)
        pos_embed = pos_embed.permute(0, 2, 3, 4, 1).view(1, -1, dim)        
        # Concatenate the class embedding back
        return torch.cat((class_emb.unsqueeze(0), pos_embed), dim=1)
